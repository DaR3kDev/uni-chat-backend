...
Esta guía está pensada para **quien nunca ha trabajado con el proyecto**. Si solo quieres un resumen técnico, ve al [README](../README.md).
...
| [Bun 1.2.23](https://bun.sh) | Solo si abres la documentación Docusaurus | `make docs` |
...
La versión del proyecto está fijada en [`docs/.bun-version`](.bun-version).
...
Abre `.env` y rellena cada variable siguiendo los comentarios de [`.env.example`](../uni-chat-backend/.env.example).
...
Solo si necesitas leer o editar la web de docs (Docusaurus):
...
Abre la URL que muestra la terminal (por defecto suele ser http://localhost:3000).
...
1. Crea `.env` con variables de producción (o usa `scripts/render-env.sh` con secrets; ver [github-secrets.md](github-secrets.md)).
...
| Bun versión incorrecta | `bun upgrade --version 1.2.23` |
...
Más detalle en el [README — Solución de problemas](../README.md#solución-de-problemas).
...
| Tema | Enlace |
|------|--------|
| Resumen del repo y Makefile | [README.md](../README.md) |
| GitHub Secrets y deploy | [github-secrets.md](github-secrets.md) |
| Endpoints y arquitectura API | [docs/backend.mdx](docs/backend.mdx) |
| Chat privado / E2E | [docs/chat-privado.md](docs/chat-privado.md) |
| CI en GitHub | [README — CI/CD](../README.md#cicd-y-calidad) |
