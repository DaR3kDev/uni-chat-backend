...
namespace uni_chat_backend.Features.Conversations.GetConversations.Handlers;
...
public class GetConversationsHandler(
    ...
    ILogger<GetConversationsHandler> logger) : IRequestHandler<GetConversationsQuery, List<GetConversationsResult>>
{
    public async Task<List<GetConversationsResult>> Handle(GetConversationsQuery request,
        CancellationToken cancellationToken)
    {
        var userId = currentUser.UserId ?? throw new UnauthorizedException("No autorizado");

        logger.LogInformation("Iniciando obtención de conversaciones para usuario: {UserId}", userId);
        ...
        var participantIds = conversations.SelectMany(c => c.Participants)
            .Where(p => p.UserId != userId)
            .Select(p => p.UserId)
            .Distinct()
            .ToList();

        logger.LogInformation("Obteniendo información de participantes. Participants: {Count}", participantIds.Count);
