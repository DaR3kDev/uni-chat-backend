    ...
    : IRequestHandler<UploadFileCommand, UploadFileResult>
{
    ...
    {
        if (request.File == null || request.File.Length == 0) throw new ArgumentException("Archivo inválido");

        var fileUrl = await cloudinaryService.UploadFileAsync(request.File);
