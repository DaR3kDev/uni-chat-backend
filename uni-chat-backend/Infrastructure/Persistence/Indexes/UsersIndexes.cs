...
public static class UsersIndexes
{
    public static async Task Create(IMongoDatabase db)
    {
        ...
        await collection.Indexes.CreateOneAsync(new CreateIndexModel<User>(
            Builders<User>.IndexKeys.Ascending(x => x.Email), new CreateIndexOptions { Unique = true }));

        await collection.Indexes.CreateOneAsync(new CreateIndexModel<User>(
            Builders<User>.IndexKeys.Ascending(x => x.Phone), new CreateIndexOptions { Unique = true, Sparse = true }));
    }
}
